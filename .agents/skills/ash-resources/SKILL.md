---
name: ash-resources
description: "Use when changing Ash resources, actions, policies, authentication, database migrations, JSON API, or Ash forms in Petitionu."
metadata:
...
```sh
mix usage_rules.search_docs "search term" -p ash -p ash_postgres -p ash_authentication -p ash_json_api -p ash_phoenix
```
