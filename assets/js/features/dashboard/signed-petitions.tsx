...
export function SignedPetitions({ signedPetitions }: { signedPetitions: Signature[] }) {
  ...
  return (
    <section
      id="dashboard-signed-petitions"
      className="app-panel"
      aria-labelledby="signed-petitions-heading"
    >
      ...
      {visibleSignatures.length === 0 ? (
        <div className="rounded-2xl bg-background px-5 py-8">
          <p className="text-sm leading-relaxed text-muted-foreground">
            You haven’t signed a petition yet. Find an idea that matters to you and add your voice.
          </p>
          <Button asChild variant="outline" className="mt-5">
            <Link to={ROUTES.petitions}>Explore petitions</Link>
          </Button>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2">
          {visibleSignatures.map(({ id, petition }) => {
            ...
            const author = petition.isAnonymous
              ? "Anonymous"
              : [petition.user?.firstName, petition.user?.lastName].filter(Boolean).join(" ")
            return (
              <article key={id} className="flex flex-col rounded-2xl border border-border p-5">
                ...
                <h3 className="text-base leading-snug font-medium">
                  <Link
                    id={`dashboard-signed-${id}`}
                    to={ROUTES.petition(petition.id)}
                    className="rounded-sm decoration-primary/40 underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"
                  >
                    {petition.title}
                  </Link>
                </h3>
                {author ? (
                  <p className="mt-2 text-xs text-muted-foreground">Started by {author}</p>
                ) : null}
