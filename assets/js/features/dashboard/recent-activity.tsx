...
export function RecentActivity({ petitions, signatures }: RecentActivityProps) {
  const activities = [
    ...
    ...signatures
      .filter((signature) => signature.petition)
      .map((signature) => ({
  ...
  return (
    ...
    >
      <h2 id="recent-activity-heading" className="mb-5 font-display text-2xl tracking-tight">
        Your recent activity
      </h2>
      {activities.length === 0 ? (
        <p className="text-sm leading-relaxed text-muted-foreground">
          When you start or sign a petition, you’ll find it here.
        </p>
      ) : (
        <ol className="divide-y divide-border">
          {activities.map((activity) => (
            <li key={activity.id} className="py-4 first:pt-0 last:pb-0">
              ...
              <Link
                to={ROUTES.petition(activity.petitionId)}
                className="mt-1 block rounded-sm text-sm leading-relaxed font-medium underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"
              >
                {activity.title}
              </Link>
              <time dateTime={activity.date} className="mt-1 block text-xs text-muted-foreground">
