...
export function UserPetitions({ petitions }: { petitions: Petition[] }) {
  ...
  return (
    <section
      ...
      className="app-panel"
      aria-labelledby="your-petitions-heading"
    >
      ...
      {petitions.length === 0 ? (
        <div className="rounded-2xl bg-background px-5 py-8">
          ...
          <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
            A better study space, a more welcoming campus. What would you like to change?
          </p>
          <Button asChild variant="outline" className="mt-5">
            <Link to={ROUTES.createPetition}>Start your first petition</Link>
          </Button>
        </div>
      ) : (
              ...
              <article key={petition.id} className="py-5 first:pt-0 last:pb-0">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  {petition.category?.name ? (
                    <Badge variant="secondary">{petition.category.name}</Badge>
                  ) : null}
                ...
                <h3 className="text-lg leading-snug font-medium">
                  <Link
                    id={`dashboard-petition-${petition.id}`}
                ...
                <div className="mt-4 mb-2 flex flex-wrap justify-between gap-2 text-xs text-muted-foreground">
                  ...
                  {daysLeft != null && Number.isFinite(daysLeft) && petition.status === "open" ? (
                    <span>
                      {daysLeft > 0
