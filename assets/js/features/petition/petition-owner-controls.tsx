...
export function PetitionOwnerControls({
  petition,
  canPublishUpdate,
}: {
  petition: Petition
  ...
  function submitLabel() {
    ...
    if (mode === "close") {
      return "Confirm close petition"
    }
    return "Confirm victory"
  }
  ...
  function renderFields() {
      ...
      return (
        <>
          <div className="space-y-2">
            <Label htmlFor="edit-petition-title">Petition title</Label>
            <Input
              id="edit-petition-title"
              value={edit.title}
              required
              maxLength={100}
          ...
          <div className="space-y-2">
            <Label htmlFor="edit-petition-description">Petition description</Label>
            <Textarea
          ...
          <div className="space-y-2">
            <Label htmlFor="edit-petition-goal">Signature goal</Label>
            <Input
              id="edit-petition-goal"
              type="number"
              min={1}
              step={1}
              required
              value={edit.goal}
              onChange={(event) => setEdit({ ...edit, goal: event.target.value })}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="edit-petition-deadline">Deadline (optional)</Label>
            <Input
              id="edit-petition-deadline"
              type="datetime-local"
              value={edit.deadline}
              onChange={(event) => setEdit({ ...edit, deadline: event.target.value })}
            />
            <p className="text-xs text-muted-foreground">
              Your local timezone. Signing and comments close at this time.
            </p>
          </div>
          <Label className="flex items-center gap-3">
            <input
              id="edit-petition-comments"
              type="checkbox"
              className="size-4 accent-primary"
              checked={edit.allowComments}
              onChange={(event) => setEdit({ ...edit, allowComments: event.target.checked })}
            />
            Allow comments
          </Label>
          <Label className="flex items-center gap-3">
            <input
              id="edit-petition-anonymous"
              type="checkbox"
              className="size-4 accent-primary"
              checked={edit.isAnonymous}
              onChange={(event) => setEdit({ ...edit, isAnonymous: event.target.checked })}
            />
            Hide my name as the petition creator
          </Label>
          <p className="text-xs leading-6 text-muted-foreground">
            Hiding your name changes the creator label to “Anonymous.” It does not hide your name on
            comments. People may already have seen or shared your name.
          </p>
        </>
      )
    }
    if (mode === "update") {
      return (
        <>
          <div className="space-y-2">
            <Label htmlFor="petition-update-title">Update title</Label>
            <Input
              id="petition-update-title"
              value={update.title}
              required
          ...
          <div className="space-y-2">
            <Label htmlFor="petition-update-body">What has changed?</Label>
            <Textarea
              id="petition-update-body"
              value={update.body}
              rows={5}
              required
          ...
          <p className="text-xs text-muted-foreground">
            Your update will appear on the petition for everyone who can view it.
          </p>
        </>
      )
    }
    return (
      <p className="text-sm leading-7">
        {mode === "close"
          ? "Close this petition? It will remain visible, but people will no longer be able to sign or comment. You can still post updates. This cannot be undone here."
          : "Has your petition achieved its goal? Marking it as a victory celebrates the result and ends signing and comments. This cannot be undone here."}
      </p>
    )
  }
  ...
  const [edit, setEdit] = useState(() => ({
    ...
    deadline: localDeadline(petition.deadline),
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (kind) => {
      function successMessage() {
        ...
        if (kind === "update") {
          return "Your update has been published."
        }
        if (kind === "close") {
          return "Your petition is closed."
        }
    ...
    >
      ...
      {mode === "idle" ? (
        <div className="flex flex-wrap gap-3">
          <Button id="edit-petition" variant="outline" onClick={() => open("edit")}>
            <Pencil aria-hidden="true" />
            Edit petition
          </Button>
          <Button
            id="add-petition-update"
            disabled={!canPublishUpdate}
            variant="outline"
            onClick={() => open("update")}
          >
          ...
          {petition.status === "open" ? (
            <Button id="close-petition" variant="outline" onClick={() => open("close")}>
              <LockKeyhole aria-hidden="true" />
              Close petition
            </Button>
          ) : null}
          {petition.status !== "victory" ? (
            <Button id="mark-petition-victory" variant="outline" onClick={() => open("victory")}>
        ...
        >
          <fieldset disabled={mutation.isPending} className="space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              <Button id={`confirm-petition-${mode}`} type="submit">
                {submitLabel()}
              </Button>
              <Button type="button" variant="outline" onClick={() => open("idle")}>
                Cancel
              </Button>
            </div>
          </fieldset>
