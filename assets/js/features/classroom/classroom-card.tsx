  ...
  return (
    <Card className="group h-full gap-0 rounded-2xl p-0 shadow-none transition-colors hover:border-primary/40">
      <Link
        ...
        className="flex flex-1 flex-col rounded-2xl p-6 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"
      >
        <div className="mb-5 flex flex-wrap gap-2">
          {isOwner && (
            <Badge variant="secondary" className="bg-[#f7e8d2] text-[#685649]">
              Professor
            </Badge>
          )}
          {classroom.archived && (
          ...
          {!classroom.allowStudentPetitions && (
            <Badge variant="outline">Professor-only petitions</Badge>
          )}
        </div>
        <h3 className="mb-3 font-display text-3xl leading-tight font-normal break-words text-foreground decoration-1 underline-offset-4 group-hover:underline">
          {classroom.name}
        </h3>
      ...
      {showJoinCode && isOwner && (
        <div className="mx-6 border-t border-border py-4">
          ...
          <code className="block text-xs break-all text-muted-foreground select-all">
            {classroom.joinCode}
          </code>
          {copyError && (
