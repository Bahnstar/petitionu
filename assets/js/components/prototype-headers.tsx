...
function Brand({ className = "" }: { className?: string }) {
  ...
  return (
    <Link
      className={`hp-brand ${className}`}
      to={`/ash-typescript${search}`}
      aria-label="PetitionU home"
    >
...
function AccountLink() {
  ...
  return isAuthenticated ? (
    <Link className="hp-account-link" to="/ash-typescript/dashboard">
      My dashboard
    </Link>
  ) : (
    <a className="hp-account-link" href="/sign-in">
      Sign in
    </a>
  )
}
  ...
  return (
    <header id="prototype-header" className="hp-site-header hp-campaign-header">
      ...
      <nav aria-label="Main navigation" className="hp-header-links">
        <a href="#prototype-petitions" aria-current="page">
          Find a cause
        </a>
        <Link to="/ash-typescript/classrooms">Your classrooms</Link>
        <span className="hp-mobile-account">
          <AccountLink />
        </span>
      </nav>
      <div className="hp-header-actions">
        ...
        <button id="prototype-header-create" type="button" onClick={startDraft}>
          Start a petition <span aria-hidden="true">+</span>
        </button>
      </div>
    </header>
  )
...
export function BoardHeader({ startDraft }: HeaderProps) {
  return (
    <header id="prototype-header" className="hp-site-header hp-board-header">
      ...
      <nav aria-label="Main navigation" className="hp-header-links">
        <a href="#prototype-petitions" aria-current="page">
          <span className="hero-squares-2x2 size-4" aria-hidden="true" />
          Campus board
        </a>
        <Link to="/ash-typescript/classrooms">
          <span className="hero-user-group size-4" aria-hidden="true" />
          Classrooms
        </Link>
      ...
      <div className="hp-header-actions">
        <AccountLink />
        <button id="prototype-header-create" type="button" onClick={startDraft}>
          <span aria-hidden="true">+</span> Start a petition
        </button>
      </div>
    </header>
  )
