...
function PetitionForm({ classroomId }: { classroomId: string | null }) {
  ...
  const classroomQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (result.success === false)
        throw new Error(
          "This classroom could not be loaded. Return to the classroom and try again.",
        )
      return result.data
    },
  })
  ...
  const categoryQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (result.success === false)
        throw new Error("We couldn't load the categories. Please try again.")
      return result.data
    },
  })
    ...
    return (
      <main id="petition-created" className="app-page">
        <section className="app-empty-state mx-auto max-w-2xl" role="status">
          <span className="mb-5 hero-check-circle size-12 text-primary" aria-hidden="true" />
          <h1 className="app-page-heading">Your idea is out there.</h1>
          <p className="app-page-description mx-auto">
            Your petition is live. Share it with the people who care, and take the next step
            together.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Button id="view-created-petition" asChild>
              <Link to={ROUTES.petition(createdId)}>View your petition</Link>
            </Button>
    ...
    <main id="create-petition-page" className="app-page">
      <header className="mb-10 max-w-2xl">
        ...
        >
          ...
          {classroomId ? "Back to classroom" : "Browse petitions"}
        </Link>
        <h1 className="app-page-heading">Let's start with your idea.</h1>
        <p className="app-page-description">
          A better campus begins with one clear ask. Tell your community what you'd change and why
          it matters.
        </p>
      </header>
      <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-12">
        <section
          className="app-panel order-2 lg:order-1"
          aria-labelledby="petition-details-heading"
        >
          <h2 id="petition-details-heading" className="mb-2 font-display text-3xl tracking-tight">
          ...
          {!authLoading && !user ? (
            <div className="mb-6 rounded-xl border border-border bg-secondary p-4 text-sm">
              Sign in to publish your petition.{" "}
          ...
          {categoryQuery.isError ? (
            <div role="alert" className="mb-6 text-sm text-destructive">
              ...
              <button type="button" className="underline" onClick={() => categoryQuery.refetch()}>
                Try again
              </button>
            </div>
          ) : null}
          ...
          <form id="create-petition-form" onSubmit={handleSubmit} className="space-y-7">
            <div className="space-y-2">
              ...
              <Input
                id="title"
                placeholder="Keep the library open later during finals"
              ...
              >
                <span>Make your title a clear, specific ask.</span>
                <span className="shrink-0">{formData.title.length}/100</span>
              </p>
            </div>
            <div className="space-y-2">
              ...
              <Select
                ...
                required
                disabled={categoryQuery.isPending || categoryQuery.isError}
              >
                <SelectTrigger id="category" className="w-full">
                  <SelectValue
                    placeholder={
                      categoryQuery.isPending ? "Loading categories…" : "Choose a category"
                    }
                  />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="description"
                placeholder="Describe the issue, the change you're asking for, and who can help make it happen."
              ...
              <p
                ...
                className="flex justify-between gap-3 text-xs text-muted-foreground"
              >
                <span>Help people understand why their support matters.</span>
                <span className="shrink-0">{formData.description.length}/2,000</span>
              </p>
            </div>
            <div className="space-y-2">
              ...
              <Select value={formData.goal} onValueChange={(value) => handleChange("goal", value)}>
                ...
                <SelectContent>
                  {[100, 250, 500, 1000, 2500, 5000, 10000].map((goal) => (
        ...
        <aside
          id="petition-writing-tips"
          className="order-1 rounded-2xl bg-[#f7e8d2] p-6 text-[#685649] lg:order-2 lg:p-7"
        >
          <span className="mb-4 hero-light-bulb size-7" aria-hidden="true" />
          <h2 className="font-display text-3xl tracking-tight">
            A little clarity goes a long way.
          </h2>
          <ul className="mt-5 space-y-4 text-sm leading-6">
            <li>Ask for one specific change.</li>
            <li>Share a personal story or a fact that makes the issue real.</li>
            <li>Name the person or department who can help.</li>
            <li>Write like you're talking to a classmate.</li>
          </ul>
        </aside>
      </div>
    </main>
