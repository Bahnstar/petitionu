...
assert.ok(
  ...
  "MVP acceptance requires a local server",
)
const fixtures = JSON.parse(
  await readFile(process.env.MVP_FIXTURES || "/tmp/petitionu-mvp-fixtures.json", "utf8"),
)
...
async function capture(page, name) {
  ...
  await page.screenshot({
    ...
    fullPage: true,
    animations: "disabled",
    style: "#tidewave-toolbar, #phx-live-reload { display: none !important; }",
  })
}
...
async function signIn(page, { email, password }) {
  ...
  await page
    .locator("#user-password-sign-in-with-password")
...
async function register(page, key, firstName, lastName) {
  ...
  await page
    .locator("#user-password-register-with-password_password_confirmation")
  ...
  const confirmation = await poll(async () => {
    ...
    const mail = data.find(
      (item) =>
        item.to.some((recipient) => recipient.includes(email)) &&
        item.subject === "Confirm your email address",
    )
  ...
  if (
    ...
    !(await page.locator("#profile-form").count())
  ) {
  ...
  await page.reload()
  await page.locator("#profile-form").waitFor()
  const user = await read(page, "get_me", {
    fields: [
      "id",
      "firstName",
      "lastName",
...
async function createPetition(page, title) {
  ...
  await page
    .locator("#description")
    .fill(
      "Keep study spaces open until midnight during finals. Students need a safe, quiet place to finish their work, with accessible entrances and reliable transport home.",
    )
...
try {
  ...
  await step("sign and comment persist", signer, async () => {
    ...
    assert.match(
      await signer.locator("#petition-detail-page").innerText(),
      /Started by Alex Morgan/,
    )
    await signer
      .locator("#signature-reason")
      .fill("A quiet place to study helps commuter students stay on track.")
    await signer.locator("#sign-petition").click()
    await signer.getByText("Your signature is counted.", { exact: false }).waitFor()
    await signer
      .locator("#petition-comment")
      .fill("Please include weekend hours and an accessible entrance.")
    await signer.locator("#post-comment").click()
    await signer
      .getByText("Please include weekend hours and an accessible entrance.", { exact: true })
      .waitFor()
    await signer.reload()
    await signer.getByText("Your signature is counted.", { exact: false }).waitFor()
    const [petition] = await read(signer, "get_petitions", {
      filter: { id: { eq: petitionId } },
      fields: ["id", "hasSigned", "signaturesCount", { comments: ["text", "author"] }],
    })
    ...
    assert.ok(
      await signer.evaluate(() => document.documentElement.scrollWidth <= innerWidth),
      "Petition page must fit a phone viewport",
    )
  ...
  await step("publish update, close and mark victory", owner, async () => {
    ...
    await owner
      .locator("#petition-update-body")
      .fill("We met with campus staff and shared the need for longer finals week hours.")
    ...
    await poll(
      async () =>
        (
          await read(owner, "get_petitions", {
            filter: { id: { eq: petitionId } },
            fields: ["status"],
          })
        )[0]?.status === "closed",
      "Petition did not close",
    )
    ...
    await poll(
      ...
      "Petition did not reach victory",
    )
    await owner.reload()
    await owner.getByRole("heading", { name: editedTitle, exact: true }).waitFor()
  ...
  await step("professor creates classroom", professor, async () => {
    ...
    await professor
      .locator("#description")
      .fill("Practice turning shared concerns into constructive campus proposals.")
    ...
    const classroom = await read(professor, "get_classroom_by_id", {
      input: { id: classroomId },
      fields: ["id", "joinCode", "name"],
    })
  ...
  await step("join classroom and display TA members", signer, async () => {
    ...
    await professor
      .getByRole("button", { name: "Demote Sam Patel to student", exact: true })
      .waitFor()
    await signer.goto(`${home}/classrooms/${classroomId}`)
    await signer
      .getByRole("button", { name: "Remove Alex Morgan from classroom", exact: true })
      .waitFor()
    ...
    await poll(
      async () =>
        (
          await read(signer, "get_memberships_for_classroom", {
            input: { classroomId },
            fields: ["userId", "status"],
          })
        ).some(
          (membership) => membership.userId === ownerUser.id && membership.status === "removed",
        ),
      "TA removal did not persist",
    )
  ...
  await step("report content through the petition page", signer, async () => {
    ...
    await signer
      ...
      .fill("Please review this synthetic acceptance example for the campus content rules.")
    ...
    const reports = await read(signer, "get_content_reports", {
      fields: ["id", "state"],
      filter: { petitionId: { eq: petitionId } },
    })
  ...
  await step("moderator reviews and hides reported content", admin, async () => {
    ...
    await poll(
      async () =>
        (
          await read(signer, "get_content_reports", {
            fields: ["id", "state"],
            filter: { id: { eq: reportId } },
          })
        )[0]?.state === "resolved",
      "Moderation decision did not persist",
    )
    await guest.reload()
    const petitions = await read(guest, "get_petitions", {
      fields: ["id"],
      filter: { id: { eq: petitionId } },
    })
    assert.deepEqual(petitions, [])
    await guest
      .getByRole("heading", { name: "We couldn't find that petition.", exact: true })
      .waitFor()
  })
  ...
  await step("submit support and account deletion requests", signer, async () => {
    ...
    await signer
      ...
      .fill("Please help me understand which campus services are available.")
    await signer.getByRole("button", { name: "Submit request", exact: true }).click()
    await signer
      .getByText("Request received. You can follow its status below.", { exact: true })
      .waitFor()
    await signer.getByRole("radio", { name: "Account deletion", exact: true }).check()
    await signer
      .locator("#support-message")
      .fill("Please review deletion of this synthetic acceptance account.")
    await signer.getByRole("button", { name: "Submit request", exact: true }).click()
    await signer
      .getByText("Request received. You can follow its status below.", { exact: true })
      .waitFor()
    await signer.reload()
    await signer
      .getByText("Please review deletion of this synthetic acceptance account.", { exact: true })
      .waitFor()
    supportRequests = await read(signer, "get_my_support_requests", {
      fields: ["id", "kind", "state"],
    })
    ...
    assert.ok(
      await signer.evaluate(() => document.documentElement.scrollWidth <= innerWidth),
      "Support page must fit a phone viewport",
    )
    await signer.setViewportSize({ width: 1440, height: 1000 })
  })
  ...
  await step("operator resolves requests without deleting the account", admin, async () => {
    ...
    await signer.reload()
    const requests = await read(signer, "get_my_support_requests", {
      fields: ["id", "state", "resolutionNote"],
    })
    ...
    await outsider
      .getByText("This queue is available to campus administrators and platform operators.", {
        exact: true,
      })
      .waitFor()
  ...
  console.log(
    JSON.stringify(
      { runId, petitionId, classroomId, ownerId: ownerUser.id, signerId: signerUser.id, results },
      null,
      2,
    ),
  )
  ...
  await Promise.all(
    contexts.map((context, index) =>
      context.storageState({ path: `/tmp/petitionu-mvp-${runId}-session-${index}.json` }),
    ),
  )
  ...
  await writeFile(
    process.env.MVP_REPORT || "/tmp/petitionu-mvp-browser-report.json",
    JSON.stringify({ runId, ...scenario, results }, null, 2),
  )
