...
The fixture script refuses a nonlocal or production database. It maintains two named synthetic campuses and three operator accounts. The browser creates fresh student accounts for each run. It writes a JSON report to `/tmp/petitionu-mvp-browser-report.json` and screenshots to the Git-ignored `docs/mvp/screenshots` directory. CI uploads screenshots with the report and server log in the `petitionu-browser-acceptance` artifact, retained for seven days. A failed run also preserves synthetic browser sessions under `/tmp` for diagnosis.
...
The final dependency-updated browser run `mtoygvia` passed all 13 scenarios. Its [JSON results](browser-results.json) are committed. The 12 desktop/mobile screenshots remain local verification artifacts and are excluded from Git. The 21 separate mocked UI checks also passed.
