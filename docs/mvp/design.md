...
Classroom membership remains pending, active, or removed. A verified actor can join by a valid unarchived code through a narrow lookup. Repeated joins return a clear already-active or awaiting-approval error without creating another row. The user can open an active classroom from their classroom list. Removed membership cannot reactivate through the old code.
